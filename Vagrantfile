...
# Install salt minion and packages for testing tinytinyrss
...
CREATE USER ttrss WITH PASSWORD 'ttrsspass';
CREATE DATABASE ttrss;
GRANT ALL PRIVILEGES ON DATABASE ttrss TO ttrss;
EOF

# Update default Apache DocumentRoot
sed -i /DocumentRoot/s/html/tinytinyrss/ /etc/apache2/sites-available/000-default.conf
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    config.vm.define "ttrss", primary: true do |host|
        host.vm.box = 'debian/contrib-buster64'
        host.vm.host_name = 'ttrss.salt.example.com'
